...
from umacircle_bot.services.legacy_identity_mapping import build_legacy_identity_mapping_manifest


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a read-only legacy Room Match identity mapping template")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        with SessionLocal() as session:
            manifest = build_legacy_identity_mapping_manifest(
                session,
        ...
        return _write_error("legacy identity mapping manifest failed")
    finally:
