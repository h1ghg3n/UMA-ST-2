...
from umacircle_bot.services.legacy_rating_chain_disposition import (
    build_legacy_rating_chain_disposition_manifest,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a read-only legacy Room Match Rating chain disposition template"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        with SessionLocal() as session:
            manifest = build_legacy_rating_chain_disposition_manifest(
                session,
        ...
        return _write_error("legacy Rating chain disposition manifest failed")
    finally:
